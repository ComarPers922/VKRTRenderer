...
layout(set = SWS_COLORS_SET, binding = 0, std430) readonly buffer ColorsBuffer
{
    vec4 Color;
} ColorsArray[];
...
void main() {
    ...
    const vec4 color = ColorsArray[nonuniformEXT(matID)].Color;
    ...
    if (color.r < 0)
    {
        texel = textureLod(TexturesArray[nonuniformEXT(matID)], uv, 0.0f).rgb;
    }
    else
    {
        texel = color.rgb;
